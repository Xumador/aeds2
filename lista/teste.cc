...
Tveiculo *inicia_noCarro()
{
    Tveiculo *cabeca = new Tveiculo;
    cabeca->prox = NULL;
    return cabeca;
}
/**
 ...
 * @param tam qunovoidade de carros
...
/**
 * @brief insere um carro no fim da lista, utiliza a função de busca, caso o carro já esteja no banco de dados e a qunovoidade
 ...
 */
ListaVeic *ordena(ListaVeic *bd)
{
    Tveiculo *aux, *tmp, *tmp2;
    aux = bd->lista;
    while (aux->prox != NULL)
    {
        tmp = aux->prox;
...
int main(int argc, char **argv)
{
    ...
    Tveiculo *cabeca = inicia_noCarro();
    Tveiculo *aux = cabeca;
    if (myfile.is_open())
    {
        while (!myfile.eof())
